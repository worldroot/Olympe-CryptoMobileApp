...
class Repository {
  ...
  var globalMetricsAPI = '${mainUrl}global-metrics/quotes/latest';
...
//A powerful Http client for Dart, which supports Interceptors, Global configuration, FormData, Request Cancellation, File downloading, Timeout etc.
  ...
  Future<ExposedGlobalMetrics> getGlobalMetrics() async {
    try {
      _dio.options.headers["X-CMC_PRO_API_KEY"] = apiKey;
      Response response = await _dio.get(globalMetricsAPI);
      return ExposedGlobalMetrics.fromJson((response.data));
    } catch (error, stacktrace) {
      // ignore: avoid_print
      print("Exception occured: $error stackTrace: $stacktrace");
      return ExposedGlobalMetrics.withError();
    }
  }
}
